...
datos_competing %>% 
  ...
  mutate(
    Comp_Descompensacio= dplyr::case_when( 
      ...
      (Descompensacio == 1 & TH == 1 & Mort == 0 ) & (Temps_Descompensacio<= Temps_TH) ~ 1,
      (Descompensacio == 1 & TH == 0 & Mort == 1 ) & (Temps_Descompensacio<= Temps_Mort) ~ 1,
      (Descompensacio == 1 & TH == 1 & Mort == 1)  & (Temps_Descompensacio<=Temps_Mort & Temps_Descompensacio <= Temps_TH & Temps_TH <= Temps_Mort)~ 1,
      (Descompensacio == 1 & TH == 1 & Mort == 1)  & (Temps_TH<=Temps_Descompensacio   & Temps_TH <= Temps_Mort  & Temps_Descompensacio <= Temps_Mort)~ 2,
      (Descompensacio == 0 & TH == 1 & Mort == 1)  & (Temps_TH <= Temps_Mort)~ 2,
      TRUE ~ 0))
# %>% 
  # mutate(
  #   Temp_Comp_TH = dplyr::case_when( 
  #     (Descompensacio == 0 & TH == 0 & Mort == 0 ) ~ ,
  #     (Descompensacio == 0 & TH == 0 & Mort == 0 ) ~ 0,
  #     (Descompensacio == 1 & TH == 1 & Mort == 1) & (Temps_Descompensacio<=Temps_Mort & Temps_Descompensacio <= Temps_TH & Temps_TH <= Temps_Mort)~ 1,
  #     (Descompensacio == 1 & TH == 1 & Mort == 1) & (Temps_TH<=Temps_Descompensacio   & Temps_TH <= Temps_Mort  & Temps_Descompensacio <= Temps_Mort)~ 2,
  #     (Descompensacio == 0 & TH == 1 & Mort == 1) & (Temps_TH <= Temps_Mort)~ 2,
